class Giveaway {
    ...
    async init() {
        this.msg = await this.channel.buildEmbed()
            .setColor(0xcc8822)
            .setTitle(this.title)
            .setDescription(`Összesen ${this.winnerCount} lehetséges nyertes van!\n Reagálj  :tada: reakcióval, hogy részt vegyél a nyereményjátékon!`)
            .setFooter(`${this.suffix} | Játék vége:`)
    ...
    async intervalFunction() {
        ...
        if(Date.now() >= this.endTime){
            ...
            if (!list.length) {
                embed.description = `Nyertes: Senki.`;
                embed.footer.text = `Vége a nyereményjátéknak!`;
            ...
            embed.description = `Nyertes: ${this.winners.filter(u => u !== undefined && u !== null).map(u => u.toString()).join(", ")}`;
            embed.footer.text = `${this.suffix} | Vége a nyereményjátéknak!`;

            this.msg.edit({ embed });

            if (this.winners.length) this.msg.channel.send(`Gratulálok, ${this.winners.map(u => u.toString()).join(", ")}! Te nyerted a **${this.title}** nevezetű nyereményjátékot!`);
        }
        ...
        /*if (this.remaining >= 5 && this.remaining % 5 === 0) {
            ...
            //const users = await this.msg.reactions.get("%F0%9F%8E%89").users.fetch();
            const users = await this.msg.reactions.get("đźŽ‰").fetchUsers();
